...
class TestSetDeterminism:
    ...
    def test_set_determinism_basic(self) -> None:
        ...
        if torch.cuda.is_available():
            ...
            assert os.environ.get("CUBLAS_WORKSPACE_CONFIG") in {None, ":4096:8"}
...
class TestGetDeterminismInfo:
    ...
    def test_get_determinism_info_structure(self) -> None:
        ...
        assert "cudnn" in info["torch_settings"]
    ...
    def test_get_determinism_info_values(self) -> None:
        ...
        cudnn_info = info["torch_settings"].get("cudnn")
        if cudnn_info is not None:
            assert cudnn_info["deterministic"] is True
            assert cudnn_info["benchmark"] is False
        if torch.cuda.is_available():
...
class TestDeterministicContext:
    ...
    def test_deterministic_context_state_restoration(self) -> None:
        ...
        # Set initial state
        cudnn_available = hasattr(torch.backends, "cudnn") and torch.backends.cudnn.is_available()
