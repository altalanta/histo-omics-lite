...
def bootstrap_confidence_intervals(
    ...
    indices_by_class = _stratify_indices(targets)
    patient_groups = _group_by_patient(patient_ids) if patient_ids is not None else None

    for i in range(n_bootstrap):
        if patient_groups:
            sampled_patients = rng.choice(list(patient_groups.keys()), size=len(patient_groups), replace=True)
            indices = np.concatenate([patient_groups[pid] for pid in sampled_patients])
        elif indices_by_class:
            sampled_chunks = [
                rng.choice(class_indices, size=len(class_indices), replace=True)
                for class_indices in indices_by_class.values()
                if len(class_indices) > 0
            ]
            indices = np.concatenate(sampled_chunks) if sampled_chunks else rng.choice(n_samples, size=n_samples, replace=True)
        else:
            indices = rng.choice(n_samples, size=n_samples, replace=True)

        if len(indices) != n_samples:
            indices = rng.choice(indices, size=n_samples, replace=True)
...
def _stratify_indices(targets: torch.Tensor) -> Dict[int, np.ndarray]:
    if targets.numel() == 0:
        return {}
    target_np = targets.detach().cpu().numpy()
    classes = np.unique(target_np.astype(int))
    return {int(cls): np.where(target_np == cls)[0] for cls in classes}


def _group_by_patient(patient_ids: Sequence[str]) -> Optional[Dict[str, np.ndarray]]:
    if not patient_ids:
        return None
    groups: Dict[str, List[int]] = {}
    for idx, pid in enumerate(patient_ids):
        groups.setdefault(pid, []).append(idx)
    return {pid: np.asarray(indices, dtype=int) for pid, indices in groups.items()}
