...
def compute_retrieval_metrics(
    ...
    # Top-k retrieval accuracy (histology -> omics)
    for k in k_values:
        topk_indices = torch.topk(similarity, k=k, dim=1)[1]
        correct = torch.any(topk_indices == torch.arange(len(histo_embeds)).unsqueeze(1), dim=1)
        accuracy = correct.float().mean().item()
        metrics[f"top{k}_histo_to_omics"] = accuracy
    
    # Top-k retrieval accuracy (omics -> histology)  
    for k in k_values:
        topk_indices = torch.topk(similarity.t(), k=k, dim=1)[1]
        correct = torch.any(topk_indices == torch.arange(len(omics_embeds)).unsqueeze(1), dim=1)
        accuracy = correct.float().mean().item()
        metrics[f"top{k}_omics_to_histo"] = accuracy
    
    # Mean reciprocal rank (MRR)
    ranks_h2o = torch.argsort(torch.argsort(similarity, dim=1, descending=True), dim=1)
    true_ranks_h2o = ranks_h2o[torch.arange(len(histo_embeds)), torch.arange(len(histo_embeds))] + 1
    mrr_h2o = (1.0 / true_ranks_h2o.float()).mean().item()
    
    ranks_o2h = torch.argsort(torch.argsort(similarity.t(), dim=1, descending=True), dim=1)
    true_ranks_o2h = ranks_o2h[torch.arange(len(omics_embeds)), torch.arange(len(omics_embeds))] + 1
